...
export const CreatePrivateButton: React.FC = () => {
    const { createRoom } = useCreationPRoom();

    return (
        <button 
            onClick={createRoom} 
            className="group relative flex items-center justify-between p-6 bg-surface-container-high hover:bg-surface-container-highest transition-all duration-200 active:scale-[0.98] border-l-0 hover:border-l-[6px] border-secondary shadow-xl overflow-hidden rounded-[18px] cursor-pointer"
        >
