  ...
  <div className="space-y-3">
    ...
    <div className="flex flex-wrap gap-2 justify-start">
      {presets.map(val => (
        <button 
          ...
          className={`w-10 h-10 rounded-xl flex items-center justify-center text-lg transition-all border cursor-pointer ${
            selected === val 
              ? 'border-oro bg-oro/10 shadow-[0_0_10px_rgba(212,175,55,0.2)]' 
      ...
      <button 
        type="button" 
        onClick={onFileClick}
        className={`w-10 h-10 rounded-xl border border-dashed flex items-center justify-center overflow-hidden transition-all cursor-pointer ${
          selected.startsWith('data:') 
            ? 'border-oro bg-oro/10' 
            : 'border-white/20 bg-white/5 hover:border-oro/40 hover:bg-oro/5'
        }`}
      >
        {selected.startsWith('data:') 
          ? <img src={selected} className="w-full h-full object-cover" alt="Avatar seleccionado" /> 
