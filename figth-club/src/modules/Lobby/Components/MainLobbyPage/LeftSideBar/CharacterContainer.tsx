...
import type { ApiUserCharacter, UserCharacter } from '../../../Types/characterTypes';
...
type CharacterWithAssets = ApiUserCharacter & { assets?: CharacterAssets };
    ...
    useEffect(() => {
        const fetchCharactersAndAssets = async () => {
            try {
                ...
                // Normalize nested UserCharacter to flat ApiUserCharacter
                let flatCharacters: ApiUserCharacter[] | null = userCharacters
                    ? userCharacters.map((c) => ({
                          id: c.id,
                          user: c.user,
                          characterId: c.character.characterId,
                          characterLevel: c.character.characterLevel,
                          characterName: c.character.characterName,
                          characterHp: c.character.characterHp,
                          characterATK: c.character.characterATK,
                          characterDEF: c.character.characterDEF,
                          characterImg: c.character.characterImg,
                      }))
                    : null;

                console.log('CharacterContainer - Personajes del prop:', flatCharacters);
                
                if (!flatCharacters || flatCharacters.length === 0) {
                    console.log('CharacterContainer - Obteniendo personajes del API...');
                    flatCharacters = await lobbyApi.getUserCharacters(userId);
                    console.log('CharacterContainer - Personajes del API:', flatCharacters);
                }
                ...
                const charactersData: CharacterWithAssets[] = await Promise.all(
                    flatCharacters.map(async (char) => {
                        try {
                            console.log(`Obteniendo assets para: ${char.characterName}`);
                            const assets = await lobbyApi.getUserCharacterAssets(userId, char.characterId.toString());
                            console.log(`Assets obtenidos para ${char.characterName}:`, assets);
                            ...
                            console.log(`Assets corregidos para ${char.characterName}:`, fixedAssets);
        ...
        <div className="flex-1 relative flex flex-col justify-end">
            ...
            <>
                ...
                <div className="absolute bottom-32 left-1/2 -translate-x-1/2 z-10">
                    <div
                        key={`${characterScreen.characterId}`}
                        className="sprite sprite-idle"
                        style={{
                            backgroundImage: `url(${characterScreen.assets?.idle_url ?? characterScreen.characterImg ?? 'https://via.placeholder.com/32x64'})`
                        }}
                    />
                </div>
                <div className="relative z-10 pb-8 pl-8 border-l-4 border-primary-container">
                    <h2 className="font-headline text-5xl font-black uppercase tracking-tighter text-on-surface leading-none mb-2"> {characterScreen.characterName}  </h2>
                    <div className="flex gap-4 mt-6">
                        <div className="bg-surface-container-highest/80 backdrop-blur px-4 py-2 border-r-2 border-secondary">
                            ...
                            <p className="font-headline text-lg">{characterScreen.characterHp}</p>
                        </div>
                        <div className="bg-surface-container-highest/80 backdrop-blur px-4 py-2 border-r-2 border-secondary">
                            <p className="text-[10px] text-stone-500 uppercase font-bold">Daño</p>
                            <p className="font-headline text-lg">{characterScreen.characterATK}</p>
                        </div>
                    </div>
                </div>
            </>
